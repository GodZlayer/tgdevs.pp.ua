...
export function createTGDeskPreview(){
  ...
  group.add(screen);
  group.visible=false;
  group.userData={canvas,ctx,texture,screen,material};
  return group;
}
...
export function updateTGDeskPreview(
  ...
  const a=clamp(alpha);
  root.visible=a>.001;
  const u=root.userData.material.uniforms;
  u.uAlpha.value=a;
  u.uAperture.value=aperture;
  u.uAspect.value=aspect;
  u.uWarp.value=warp;
  u.uEdge.value=1-mix(aperture,.86,1.08);

  root.userData.screen.scale.set(width,height,1);
  ...
  // returning home has one last subtle selection pulse as the remote session
  // collapses back into the same device node that launched it.
